...
- WEEKLY PROGRESSION (applies to EVERY template): If the template has a "weeklyProgression.weeks" array, that array dictates how primary strength blocks evolve week-by-week. Each entry has { week, sets?, reps?, intensityPct, intent }. For each week of the program:
   1. If the entry has explicit "sets" and "reps", PRIMARY strength blocks (intensity:"primary" with a pair label like 1a/1b, OR a standalone main lift) use those exact numbers — NOT the block-level defaults, NOT a range.
   2. The "intensityPct" is the load target as % of the athlete's e1RM. For each primary exercise, compute the prescribed load as: load_kg = round(e1RM × intensityPct / 100) (use the most recent e1RM from the athlete snapshot's strengthProgression). Include this in the exercise's "loadCue" field, e.g. "loadCue": "85 kg (≈80% e1RM)".
   3. Secondary blocks scale with the same intensityPct (apply to their own loaded exercises) but keep their template-defined sets/reps.
   4. The "intent" tag (intro / build / peak / deload / load) gives a one-word coaching purpose — surface this in the week's "theme" field.
   5. weeklyProgression OVERRIDES the block-level sets/reps for primary work in templates that DON'T have undulating or wave schemes. For undulating/wave templates, weeklyProgression's intensityPct still drives loads, but sets/reps come from dailyRotation or wavePeriodization.
